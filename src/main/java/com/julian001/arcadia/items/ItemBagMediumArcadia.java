...
public class ItemBagMediumArcadia extends ItemArcadia
{
	public ItemBagMediumArcadia(String name)
	{
		super(name);
		this.maxStackSize = 1;
	}
	...
	*/
	...
	player.getHeldItem() != null && player.getHeldItem().getItem() instanceof ItemBagMediumArcadia)
	{
	// Open the correct GUI for the player at player's position
	player.openGui(arcadia.instance, GuiIds.guiBagMediumIndex, world, (int) player.posX, (int) player.posY, (int) player.posZ);
	}
	
	// If our ContainerItem is currently open, write contents to NBT when needsUpdate is true
	if(player.openContainer != null && player.openContainer instanceof ContainerBagMediumArcadia
	&& ((ContainerBagMediumArcadia) player.openContainer).needsUpdate)
	{
	((ContainerBagMediumArcadia) player.openContainer).writeToNBT();
	// Set needsUpdate back to false so we don't continually write to NBT
	((ContainerBagMediumArcadia) player.openContainer).needsUpdate = false;
	}
	}
	}
	/*
