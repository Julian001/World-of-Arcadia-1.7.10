...
public class ItemBagSmallArcadia extends ItemArcadia
{
	public ItemBagSmallArcadia(String name)
	{
		super(name);
		this.maxStackSize = 1;
	}
	...
	*/
	...
	player.getHeldItem() != null && player.getHeldItem().getItem() instanceof ItemBagSmallArcadia)
	{
	// Open the correct GUI for the player at player's position
	player.openGui(arcadia.instance, GuiIds.guiBagSmallIndex, world, (int) player.posX, (int) player.posY, (int) player.posZ);
	}
	
	// If our ContainerItem is currently open, write contents to NBT when needsUpdate is true
	if(player.openContainer != null && player.openContainer instanceof ContainerBagSmallArcadia
	&& ((ContainerBagSmallArcadia) player.openContainer).needsUpdate)
	{
	((ContainerBagSmallArcadia) player.openContainer).writeToNBT();
	// Set needsUpdate back to false so we don't continually write to NBT
	((ContainerBagSmallArcadia) player.openContainer).needsUpdate = false;
	}
	}
	}
	/*
