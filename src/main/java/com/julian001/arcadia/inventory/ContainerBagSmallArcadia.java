...
public class ContainerBagSmallArcadia extends Container
{
	/** The Item Inventory for this Container */
	public final InventoryBagSmallArcadia inventory;
	...
	* InventoryItem.INV_SIZE and if we ever change it, the Container updates automatically. */
	private static final int INV_START = InventoryBagSmallArcadia.INV_SIZE, INV_END = INV_START+26,
...
public ContainerBagSmallArcadia(EntityPlayer par1Player, InventoryPlayer inventoryPlayer, InventoryBagSmallArcadia inventoryItem)
{
	...
	for (i = 0; i < InventoryBagSmallArcadia.INV_SIZE; ++i)
	{
	...
	this.addSlotToContainer(new SlotBagArcadia(this.inventory, i, 62 + (18 * (int)(i/3)), 11 + (18*(i%3))));
	}
	...
	*/
	...
	for (int j = 0; j < 9; ++j)
	{
	this.addSlotToContainer(new Slot(inventoryPlayer, j + i * 9 + 9, 8 + j * 18, 78 + i * 18));
	}
	}
	...
	*/
	...
	// Cast to ItemArcadiaBag so we can call the method from that class:
	((InventoryBagSmallArcadia) inventory).writeToNBT(this.containerstack.getTagCompound());
	}
